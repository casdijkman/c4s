...
export class DomSurfer {
  ...
  add(value) {
    ...
    this.elements = this.elements.filter(
      (element) => DomSurfer.isElement(element)
    ).filter(
  ...
  hasClass(cssClass) {
    ...
    return cssClass instanceof RegExp
      ? DomSurfer.elementClassesFromRegex(element, cssClass).length > 0
  ...
  data(name) {
    const element = this.first();
    return name in element.dataset ? element.dataset[name] : this.attr(`data-${name}`);
  }
  ...
  removeClass(cssClass) {
    return this.each((element) => {
      const classes = cssClass instanceof RegExp
        ? DomSurfer.elementClassesFromRegex(element, cssClass)
        : [cssClass];
